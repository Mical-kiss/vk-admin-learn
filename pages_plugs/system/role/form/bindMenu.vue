  ...
  >
    ...
    >
      <template v-slot:menuList>
        <el-input placeholder="输入关键字进行搜索" v-model="tree.filterText">
        </el-input>
        <view style="margin-top: 5px">
          <el-checkbox v-model="tree.expandAll" @change="expandAllChange"
            >展开/折叠</el-checkbox
          >
          <el-checkbox
            v-model="tree.checkAll"
            @change="checkAllChange"
            v-if="!tree.filterText"
            >全选/全不选</el-checkbox
          >
        </view>
        <view class="tree-box">
    ...
    onFormSuccess(res) {
      if (res.data.info) {
        ...
        that.$set(
          that.value.item,
          "stats_count_info",
          res.data.info.stats_count_info
        );
      }
